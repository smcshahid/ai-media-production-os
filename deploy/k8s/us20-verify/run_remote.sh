...
export RUN_ID="${RUN_ID:?set RUN_ID to a COMPLETED pipeline run (e.g. US-V02 042983f7-...)}"
LOG="/tmp/us20-verify-$(date +%Y%m%d-%H%M%S).log"
bash /tmp/verify_us20.sh 2>&1 | tee "$LOG"
RC=${PIPESTATUS[0]}
