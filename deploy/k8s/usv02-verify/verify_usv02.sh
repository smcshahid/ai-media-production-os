...
curl -s -m 60 -X POST "$API/pipeline/approve" -H "$AUTH" -H 'Content-Type: application/json' \
  -d '{"project_id":"'"$PROJECT"'","stage":"SCRIPT","decision":"REJECT","note":"US-V02: tighten dialogue in the second scene."}'
echo
...
curl -s -m 60 -X POST "$API/pipeline/approve" -H "$AUTH" -H 'Content-Type: application/json' \
  -d '{"project_id":"'"$PROJECT"'","stage":"STORYBOARD","decision":"REJECT","note":"US-V02 A-01: increase contrast and wider establishing shots."}'
echo
...
curl -s -m 60 -X POST "$API/pipeline/approve" -H "$AUTH" -H 'Content-Type: application/json' \
  -d '{"project_id":"'"$PROJECT"'","stage":"VIDEO","decision":"REJECT","note":"US-V02: slightly longer hold on frame 2."}'
echo
