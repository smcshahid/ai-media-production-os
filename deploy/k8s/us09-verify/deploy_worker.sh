#!/usr/bin/env bash
# Import US-09 worker image and rollout on Olares.
set -euo pipefail
NS=aimpos-mwayolares
IMAGE="${AIMPOS_WORKER_IMAGE:-docker.io/library/aimpos-worker:us09}"
TAR="${WORKER_TAR:?set WORKER_TAR to path of aimpos-worker-us09.tar on Olares node}"
K="sudo k3s kubectl"
