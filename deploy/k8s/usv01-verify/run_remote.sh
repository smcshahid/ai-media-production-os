...
LOG="/tmp/usv01-verify-$(date +%Y%m%d-%H%M%S).log"
bash /tmp/verify_usv01.sh 2>&1 | tee "$LOG"
RC=${PIPESTATUS[0]}

# Export RUN_ID from log if verify did not export
if [ -z "${RUN_ID:-}" ]; then
...
echo "========== SQL collect =========="
export EVIDENCE_SQL_DIR="/tmp/usv01-sql-$(date +%Y%m%d)"
bash /tmp/collect_usv01.sh 2>&1 | tee "/tmp/usv01-collect.log"
