#!/usr/bin/env bash
# Post-run SQL attestation → evidence/us-v01-verification/olares-<date>/sql/
set -euo pipefail
...
: "${RUN_ID:?set RUN_ID}"
K="sudo k3s kubectl"
OUT="${EVIDENCE_SQL_DIR:-/tmp/usv01-sql}"
mkdir -p "$OUT"
...
run_q "v08-story-timing" "SELECT event_type, payload->>'stage', created_at FROM audit_events WHERE pipeline_run_id='$RUN_ID' ORDER BY created_at LIMIT 30;"
