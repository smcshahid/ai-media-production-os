#!/usr/bin/env bash
# US-V08B — deploy Phase 7.5 character hardening images to Olares.
set -euo pipefail
...
$K delete pod -l app=aimpos-worker -n "$NS" --wait=true
$K rollout status deployment/aimpos-worker -n "$NS" --timeout=300s
...
$K delete pod -l app=aimpos-web -n "$NS" --wait=true
$K rollout status deployment/aimpos-web -n "$NS" --timeout=300s
