# US-V08B Olares acceptance orchestrator (Phase 7.5)
param(
...
$EvidenceRoot = Join-Path $RepoRoot "evidence/us-v08b-verification/olares-$Date"
...
$scripts = @(
    ...
    "deploy/k8s/usv08b-verify/apply_0007_sql_olares.sh",
    "deploy/k8s/usv08b-verify/deploy_usv08b.sh",
    "deploy/k8s/usv08b-verify/verify_usv08b_e2e.sh"
)
...
$tarPath = Join-Path $env:TEMP "aimpos-usv08b-src.tgz"
if (Test-Path $tarPath) { Remove-Item $tarPath -Force }
tar --exclude=node_modules --exclude=.git --exclude=evidence --exclude=__pycache__ -czf $tarPath -C $RepoRoot .
scp -q $tarPath "${OlaresHost}:/tmp/aimpos-usv08b-src.tgz"
ssh -o BatchMode=yes $OlaresHost "rm -rf /tmp/aimpos-usv08b-src && mkdir -p /tmp/aimpos-usv08b-src && tar -xzf /tmp/aimpos-usv08b-src.tgz -C /tmp/aimpos-usv08b-src"
Log "Source synced to /tmp/aimpos-usv08b-src"
...
Log "=== Apply migration 0007 ==="
ssh -o BatchMode=yes $OlaresHost "bash /tmp/apply_0007_sql_olares.sh" 2>&1 | Tee-Object -FilePath (Join-Path $LogDir "migration-0007.log")

if (-not $SkipBuild) {
    $Tag = "usv08b-phase75"
    $TarDir = Join-Path $env:TEMP "aimpos-usv08b-tars"
    New-Item -ItemType Directory -Force -Path $TarDir | Out-Null
    $ErrorActionPreference = "Continue"

    Log "=== Build usv08b-phase75 images locally ==="
    ...
    Log "=== Deploy usv08b-phase75 ==="
    $ErrorActionPreference = "Continue"
    ssh -o BatchMode=yes $OlaresHost "chmod +x /tmp/deploy_usv08b.sh /tmp/verify_usv08b_e2e.sh /tmp/apply_0007_sql_olares.sh && bash /tmp/deploy_usv08b.sh" 2>&1 | Tee-Object -FilePath (Join-Path $LogDir "deploy-usv08b.log")
...
Log "=== US-V08B E2E (Paths A-F) - long running ==="
$ErrorActionPreference = "Continue"
$remote = @'
...
Log "=== Fetch evidence ==="
foreach ($subdir in @("path-a","path-b","path-c1","path-c2","path-d","path-e","path-f")) {
    New-Item -ItemType Directory -Force -Path (Join-Path $EvidenceRoot $subdir) | Out-Null
}
scp -r -q "${OlaresHost}:/tmp/usv08b-evidence/*" $EvidenceRoot 2>$null
